...
const GoModGuide: FC = () => {
  ...
  const change = (event)=>{
    ...
    setSelectedLangDict(LangDict[lang]);
  }

  const hl = (child: any) => <span className="hl">{child}</span>
  return <>
    <Head>
      <title>{selectedLangDict['title']}</title>
      <meta name="twitter:title" content="{selectedLangDict['title']} • Encore" />
    ...
    <div className="min-h-screen w-full bg-gray-200 flex justify-center items-center sm:p-4">
      <div className="w-full bg-white sm:rounded-lg max-w-screen-lg">
        <header className="lg:h-28 sm:rounded-t-lg text-white relative p-2">
          <div>
              <div className="absolute top-0 right-0 flex items-center bottom-0 right-2 ">
                <label  className="sr-only">Language</label>
                <select  onChange={change} value={selectedLangDict['lang']} id="language" name="currency" className="focus:ring-indigo-200 v-full border-transparent bg-transparent text-gray-200 sm:text-sm rounded-md">
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <h1 className="text-xl lg:text-5l">{selectedLangDict['title']}</h1>
          </div>
        ...
        <div className="p-2 lg:p-8 text-sm lg:text-base max-w-screen-lg overflow-auto">
          <section>
            <h2 className="text-xl lg:text-4xl mb-6">{selectedLangDict["quick_start"]}</h2>
            <pre className="font-mono leading-none">

              <div className="text-gray-400">{selectedLangDict['dep_management']}</div>
              <div className="text-gray-400 block sm:hidden mt-3">{selectedLangDict['add_dep']}</div>
              go get -d github.com/path/to/module       <span className="text-gray-400 hidden sm:inline">{selectedLangDict['add_dep']}</span><br />
              <div className="text-gray-400 block sm:hidden">{selectedLangDict['specific_version']}</div>
              go get -d github.com/dep/two/v2{hl("@v2.1.0")}    <span className="text-gray-400 hidden sm:inline">{selectedLangDict['specific_version']}</span><br />
              <div className="text-gray-400 block sm:hidden">{selectedLangDict['specific_branch']}</div>
              go get -d github.com/dep/commit{hl("@branch")}    <span className="text-gray-400 hidden sm:inline">{selectedLangDict['specific_branch']}</span><br />
              <div className="text-gray-400 block sm:hidden">{selectedLangDict['upgrade_modules']}</div>
              go get -d -u ./...                        <span className="text-gray-400 hidden sm:inline">{selectedLangDict['upgrade_modules']}</span><br /><br />

              <div className="text-gray-400 block sm:hidden">{selectedLangDict['remove_dep']}</div>
              go get -d github.com/dep/legacy{hl("@none")}      <span className="text-gray-400 hidden sm:inline">{selectedLangDict['remove_dep']}</span><br /><br />

              <div className="text-gray-400 mt-3 sm:mt-0">{selectedLangDict['useful_commands']}</div>
              <div className="text-gray-400 block sm:hidden mt-3">{selectedLangDict['tidy_mod']}</div>
              go mod tidy                               <span className="text-gray-400 hidden sm:inline">{selectedLangDict['tidy_mod']}</span><br />
              <div className="text-gray-400 block sm:hidden mt-3">{selectedLangDict['download_mod']}</div>
              go mod download                           <span className="text-gray-400 hidden sm:inline">{selectedLangDict['download_mod']}</span><br />
              <div className="text-gray-400 block sm:hidden mt-3">{selectedLangDict['init_module']}</div>
              go mod init github.com/path/to/module     <span className="text-gray-400 hidden sm:inline">{selectedLangDict['init_module']}</span><br />
              <div className="text-gray-400 block sm:hidden mt-3">{selectedLangDict['why_dep']}</div>
              go mod why -m github.com/path/to/module   <span className="text-gray-400 hidden sm:inline">{selectedLangDict['why_dep']}</span><br />
              <div className="text-gray-400 block sm:hidden mt-3">{selectedLangDict['install_binary']}</div>
              go install github.com/path/to/bin{hl("@latest")}  <span className="text-gray-400 hidden sm:inline">{selectedLangDict['install_binary']}</span>
            </pre>
          </section>

          <section className="mt-6 lg:mt-10">
            <h2 className="text-xl lg:text-4xl mb-6">{selectedLangDict['anatomy_mod']}</h2>
            <Anatomy selectedLangDict={selectedLangDict} />
          </section>

          <section className="mt-6 lg:mt-10">
            <h2 className="text-xl lg:text-4xl mb-6">{selectedLangDict['mvs']}</h2>
            <div className="flex flex-col gap-y-4">
              <p>{selectedLangDict['mvs_p1']}</p>
              <p>{selectedLangDict['mvs_p2']}</p>
              <p>{selectedLangDict['mvs_p3']}</p>
              <ol className="list-inside list-decimal pl-4">
                <li>{selectedLangDict['mvs_ol_li1']}</li>
                <li>{selectedLangDict['mvs_ol_li2']}</li>
                <li>{selectedLangDict['mvs_ol_li3_1']}<code>{selectedLangDict['mvs_ol_li3_2']}</code>{selectedLangDict['mvs_ol_li3_3']}</li>
              </ol>
            </div>
            <h3 className="text-2xl mt-6 mb-4">{selectedLangDict['example']}</h3>
            <div className="flex flex-col lg:flex-row gap-4 text-sm">
              <div>
                <img src="/mvs.svg" className="mx-auto h-32 lg:h-48 w-auto" />
              </div>
              <div className="flex flex-col gap-y-4">
                <p dangerouslySetInnerHTML={{__html:selectedLangDict['mvs_example_p1']}}></p>

                <p dangerouslySetInnerHTML={{__html:selectedLangDict['mvs_example_p2']}}></p>

                <p dangerouslySetInnerHTML={{__html:selectedLangDict['mvs_example_p3']}}></p>

                <p>{selectedLangDict['mvs_example_p4']}
                  <ul className="list-inside list-disc">
                    <li><code>{selectedLangDict['mvs_example_p4_ul_li1']}</code></li>
                    <li><code>{selectedLangDict['mvs_example_p4_ul_li2']}</code></li>
                    <li><code>{selectedLangDict['mvs_example_p4_ul_li3']}</code></li>
                  </ul>
                </p>
              </div>
            </div>
        ...
        <div className="text-center py-2 text-xs">
          ...
          <div>{selectedLangDict['contributions']}<a className="text-indigo-600 hover:text-indigo-400" href="https://github.com/encoredev/cheatsheets" rel="nofollow" target="_blank">https://github.com/encoredev/cheatsheets</a> (MIT License)</div>
        </div>
      </div>
    </div>
  </>
  ...
  return (
    <svg className="w-full h-auto" viewBox="0 0 781.04 387.45">
      ...
      <g transform="translate(420.9 9)">
        <text x="0" y="0" className="note">{selectedLangDict['module_path']}</text>
      </g>

      <g transform="translate(420.9 44.5)">
        <text x="0" y="0" className="note">
          <tspan x="0" dy="0">{selectedLangDict['module_version']}</tspan>
          <tspan x="0" dy="1.25em">{selectedLangDict['new_features']}</tspan>
        </text>
      </g>

      <g transform="translate(420.9 92.5)">
        <text x="0" y="0" className="note">{selectedLangDict['v2_later_version']}</text>
      </g>

      <g transform="translate(560.9 135)">
        <text x="0" y="0" className="note">
          <tspan x="0" dy="0">{selectedLangDict['pseudo_version_1']}</tspan>
          <tspan x="0" dy="1.25em">{selectedLangDict['pseudo_version_2']}</tspan>
        </text>
      </g>

      <g transform="translate(553 247)">
        <text x="0" y="0" className="note">
          <tspan x="0" dy="0">{selectedLangDict['incompatible_package_1']}</tspan>
          <tspan x="0" dy="1.25em">{selectedLangDict['incompatible_package_2']}</tspan>
        </text>
      </g>

      <g transform="translate(211 363.5)">
        <text x="0" y="0" className="note">{selectedLangDict['replace_module']}</text>
      </g>

      <g transform="translate(15 355)">
        <text x="0" y="0" className="note">
          <tspan x="0" dy="0">{selectedLangDict['specific_version_1']}</tspan>
          <tspan x="0" dy="1.25em">{selectedLangDict['specific_version_2']}</tspan>
          <tspan x="0" dy="1.25em">{selectedLangDict['specific_version_3']}</tspan>
        </text>
      </g>
